...
RSpec.describe AmstradGpt::Gateway do
  ...
  describe ".run" do
    it "creates a new instance and calls run on it" do
      expect(described_class).to receive(:new).with(api_key:, tty:).and_return(subject)
      expect(subject).to receive(:run)
      described_class.run(api_key:, tty:)
    end
  end
  ...
  describe "#forward_query" do
    it "sends the message to ChatGpt and replies with the response" do
      ...
      subject.send(:forward_query, message)
    end
  end
  ...
  describe "#chat_gpt" do
    it "initializes ChatGpt instance with api_key and prompt" do
      expect(AmstradGpt::ChatGpt).to receive(:new).with(api_key:,
                                                        prompt: described_class::PROMPT).and_return(chat_gpt)
